...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        return False;
    if len(t.branches) == 2:
        if max_t(t.branches[0]) > t.label:
            return False;
        if min_t(t.branches[1]) <= t.label:
            return False;
        for b in t.branches:
            if not is_bst(b):
                return False;
...
def in_order_traversal(t):
    ...
    def helper(t):
        ...
        if len(t.branches) == 2:
            result += in_order_traversal(t.branches[0]);
            result += [t.label];
            result += (in_order_traversal(t.branches[1]));
